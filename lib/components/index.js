...
const groupMethodsWithTypes = entries => {
    ...
    return (
        entries
            ...
            .reduce(
                (prev, curr) =>
                    methods.hasOwnProperty(curr.name)
                        ? [...prev, curr, ...methods[curr.name].sort((a, b) => a.line - b.line)]
                        : [...prev, curr],
                [],
            )
    );
...
export default class View extends Component {
    ...
    navToItem(line) {
        atom.workspace.getCenter().observeActivePaneItem(item => {
            if (typeof item === undefined || item === null || typeof item.setCursorScreenPosition !== "function") {
                return;
            }
    ...
    render() {
        const containerStyle = { height: "100%", overflowY: "auto", paddingTop: "1.3em" },
            headerWrapperStyle = {
                position: "absolute",
                margin: ".5em",
                top: 0,
            ...
            headerContainerStyle = { backgroundColor: "#21252b" },
            headerTextStyle = { fontWeight: 600, overflow: "hidden", whiteSpace: "nowrap" },
        ...
        const { version, entries } = this.props;

        if (version !== null && version !== compatibleVersion) {
                    ...
                    <ul style={ulStyle}>
                        {this.sort(entries).map((e, i) => (
                            <li
                                key={i}
                                style={liStyle}
                                onMouseEnter={() => {}} /* eventually make this change the background color */
                            >
                                <span style={lineNumStyle}>{`${e.line}:`}</span>
                                <button style={{ ...asText, ...lineItemBtn }} onClick={() => this.navToItem(e.line)}>
                                    <span
                                        style={htmlSpanStyle}
                                        className="syntax--source syntax--go"
                                        dangerouslySetInnerHTML={{ __html: this.stylize(e) }}
                                    />
                                </button>
                            </li>
                        ))}
