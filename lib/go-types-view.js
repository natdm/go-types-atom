...
export default class GoTypesView {
    constructor(serializedState) {
        ...
        if (this.execExists()) {
            exec(`go-typelist -v`, (err, stdout, stderr) => {
                if (err !== null) {
                    ReactDOM.render(<Error error={'Error checking go-typelist version. Attempting to update'} />, this.element);
                }
                if (err !== null || stdout.trim() !== compatibleVersion.trim()) {
                    this.getExec((err, res) => {
                        if (err !== null) {
                            console.error('error updating go-typelist: ', err);
                            ReactDOM.render(<Error error={'Error getting go-typelist'} />, this.element);
                            return;
                        }
                        ReactDOM.render(<LoadingMessage message={'Done Go Getting -u. Select your file again.'} />, this.element);
    ...
    setListeners() {
        ...
        atom.workspace.observeTextEditors(editor => {
            console.log('is softwrapped: ', editor.isSoftWrapped());
    ...
    parseFile() {
        ...
        if ((editor = atom.workspace.getActivePaneItem())) {
            ...
            exec(`go-typelist ${file.path}`, (err, stdout, stderr) => {
                ...
                ReactDOM.render(<View entries={entries.objects} />, this.element);
            });
        }
    }
