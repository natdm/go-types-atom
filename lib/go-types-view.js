...
export default class GoTypes {
    constructor(serializedState) {
        ...
        // Create mounted element
        console.log(atom.workspace.getCenter());
    ...
    setListeners() {
        atom.workspace.observeTextEditors(editor => editor.onDidSave(arg => this.parseFile()));
    }
            ...
            exec(`goparsetypes ${file.path}`, (err, stdout, stderr) => {
                ...
                if (stderr !== '') {
                    if (stderr.indexOf('command not found') > -1) {
                        ...
                        ReactDOM.render(
                            <Error error={"Install Go Typelist with 'go get github.com/natdm/go-typelist' and then 'go install'"} />,
                            this.options.item,
                        );
                        return;
                    }
                ...
                if (entries.version !== currentVersion) {
                    ReactDOM.render(<UpgradeComponent />, this.element);
                    return;
                }
